...
今までの章では presets に用意された package.json や設定ファイルを利用して解答を作成してきたが、この章では ch17 ディレクトリの下にある `package.json` を使用する。各問題の回答を進める上で必要な内容を `package.json` に適宜追記しなさい。また、その他の設定ファイルも必要に応じてを整備しなさい。
